---
title: "MicroSD Card as a Second Factor for Coldcard Login"
source: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/microsd-2fa.md"
type: articles
ingested: 2026-08-10
tags: [coldcard, 2fa, microsd, aes, serial-number-binding, seed-wipe, login-security, q-dual-slot]
summary: "Explains the MicroSD second-factor login option: once enabled, a specially prepared card must be present during login, and if the slot is empty or holds an unknown card after the correct PIN is entered, the seed is wiped. Enrollment writes a small encrypted file to the card; at login the master secret is used to build an AES key that decrypts it, and if the result is JSON containing a nonce on the acceptable list, login proceeds. The AES key incorporates both the master secret and a hash of the card's unique serial number retrieved via low-level protocols, so the file cannot simply be copied to another card. To let one card unlock several Coldcards, the filename is derived from a hash of the device's serial number, allowing multiple .2fa files to coexist; the name begins with a dot so tools may hide it, and reformatting the card removes it. Also notes the Coldcard Q behaviour when both card slots are populated during login."
collection: "coldcard"
adapter: git
upstream_id: "docs/microsd-2fa.md"
upstream_type: git-file
revision: "43b2139227149c281141d08c612afd13c434d456"
sha: "9da406225cea359336b28baa8eed4241a8b21be6"
canonical_url: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/microsd-2fa.md"
