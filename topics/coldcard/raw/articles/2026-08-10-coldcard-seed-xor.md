---
title: "Seed XOR: Plausibly Deniable Split Seed Backups"
source: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/seed-xor.md"
type: articles
ingested: 2026-08-10
tags: [coldcard, seed-xor, bip-39, seed-backup, plausible-deniability, duress, shamir-alternative, xor, seedplate]
summary: "Presents Seed XOR, a scheme for splitting one 12-, 18- or 24-word BIP-39 seed into two or more parts that are themselves valid BIP-39 seeds of the same length, XOR'ed together bit by bit to reconstruct the original. Motivated by the metal-backup problem: plates survive fire but store cleartext secrets that anyone with physical access can spend, while encrypted digital backups are neither discreet nor immune to compelled disclosure. The stated properties are that every part looks and behaves like an ordinary wallet, parts combine in any order, all parts are required because any strict subset is itself a valid (different) Seed XOR wallet, no new recording tools are needed since each part fits a normal SEEDPLATE, and up to N-1 parts leak nothing about the original. Each part carries a normal BIP-39 checksum in its final word protecting that part's integrity, but those checksum bits are excluded from the XOR (8 bits for 24 words, 4 for 12). Because each part can hold honeypot funds, the scheme doubles as a duress tool. Coinkite recommends storing the original wallet's checksum word alongside the parts to confirm correct reassembly, while noting this reveals 3 bits of the real wallet and confirms when a correct subset has been assembled. Includes an XOR lookup table, an open-standard note, and fully worked 3-part examples for both 24-word and 12-word seeds so the computation can be done and verified on paper. Also answers the lazy case: an existing Coldcard seed can be split into 2, 3 or 4 new plates, or existing plates can be combined to create a new random wallet."
collection: "coldcard"
adapter: git
upstream_id: "docs/seed-xor.md"
upstream_type: git-file
revision: "43b2139227149c281141d08c612afd13c434d456"
sha: "be32bb7627639a4552aed5a1e1791848eef7dd70"
canonical_url: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/seed-xor.md"
