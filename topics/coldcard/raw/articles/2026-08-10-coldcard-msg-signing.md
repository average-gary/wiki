---
title: "Coldcard Message Signing and Signed Exports"
source: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/msg-signing.md"
type: articles
ingested: 2026-08-10
tags: [coldcard, message-signing, bip-137, signed-exports, detached-signature, nfc, qr, verification, script-types]
summary: "Documents message signing over USB (via the ckcc utility), by specially crafted file on SD card or virtual disk, and over NFC on the NFC-equipped Mk4, Mk5 and Q; signatures return via SD/Vdisk, NFC, or as a QR code on the Q. Signatures follow BIP-137, with an important compatibility note: Mk3 and Mk4 up to version 5.1.0 used the compressed P2PKH header byte for all script types, while 5.1.0 onward uses the correct header byte per script type. From 5.1.0 the device can also verify signed messages itself - via Advanced/Tools > File Management > Verify Sig File for SD/Vdisk, or Advanced/Tools > NFC Tools > Verify Sig File for NFC - and when the signature is a detached signature over an exported file it can confirm the digest named in the message matches the file's contents, which requires the signed file to be present; signature files are limited to about 10KB. Cross-verification is suggested via verifybitcoinmessage.com because Bitcoin Core can only verify P2PKH. Also specifies Signed Exports, where from 5.1.0 most SD and virtual-disk exports ship with a detached .sig alongside (addresses.csv paired with addresses.sig), and gives the exact message construction: single SHA256 of the file contents, then msg = that hash + two spaces + the export's basename, then the standard Bitcoin signed-message hash over msg."
collection: "coldcard"
adapter: git
upstream_id: "docs/msg-signing.md"
upstream_type: git-file
revision: "43b2139227149c281141d08c612afd13c434d456"
sha: "6907e8825b824515b6da011bd167ed0f36ea8153"
canonical_url: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/msg-signing.md"
