---
title: "Coldcard Web 2FA Authentication"
source: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/web2fa.md"
type: articles
ingested: 2026-08-10
tags: [coldcard, web2fa, totp, rfc-6238, no-rtc, spending-policy, hsm, hotp, ecc-encryption, qr]
summary: "Answers how a device with no real-time clock can support RFC 6238 TOTP two-factor authentication: the pre-shared secret lives on the Coldcard and is sent, encrypted, to a trusted webserver that does know the time and can offer a richer UX. That server takes the time-based code from the user and, if correct, reveals a secret the Coldcard can use to authorize an action - an 8-digit numeric code to type on Mk4, or a QR code to scan on the Q. Notes the historical contrast with the HSM feature's HOTP tokens, which need no backend but are less robust, and that Web2FA can be enabled as part of a Spending Policy in either multisig or single-signer mode, prompting for 2FA after the transaction details are displayed and blocking signing without the correct code. The mechanism keeps the server stateless: it holds an ECC keypair whose pubkey ships in Coldcard firmware releases, the base32 TOTP secret is chosen and stored by the Coldcard, and the Coldcard builds a URL encrypted to the server's pubkey carrying the shared secret and other arguments."
collection: "coldcard"
adapter: git
upstream_id: "docs/web2fa.md"
upstream_type: git-file
revision: "43b2139227149c281141d08c612afd13c434d456"
sha: "5fa88d6a82a0699220c7b891b49e8bbbffa93490"
canonical_url: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/web2fa.md"
