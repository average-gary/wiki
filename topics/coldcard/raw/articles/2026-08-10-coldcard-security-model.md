---
title: "Coldcard Mk4/Mk5/Q Security Model"
source: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/security-model.md"
type: articles
ingested: 2026-08-10
tags: [coldcard, security-model, threat-model, secure-element, trick-pins, delta-mode, duress-wallet, bip-85, flash-ecc, fast-wipe]
summary: "The central threat-model document. Traces the progression from Mk1-Mk3 (a single Microchip ATECC508A/608B secure element holding 72 bytes behind a 4-12 digit PIN) to Mk4's dual-secure-element design, whose stated goal is that SE1, SE2 and the MCU must all be compromised before the seed leaks. Catalogues the Trick PIN options (wipe seed, look blank, duress wallet, brick self, login countdown up to 28 days restarting on power cycle, pretend-wrong-PIN, wiped message, reboot, Delta Mode), the BIP-85 duress-wallet derivations (accounts 1001/1002/1003 for 24-word, 2001/2002/2003 for 12-word), and Delta Mode, which differs from the True PIN only in its last four digits and produces deliberately invalid signatures to defend against an attacker who already knows the XPUB/XFP. Also covers the Kill Key, removal of the SPI serial flash in favour of AES-encrypted settings in MCU flash, PSBT handling in 8MB PSRAM enabling 1MB transactions, replacement of the virtual debug serial port with a physical UART that requires case damage to reach, SD Card Recovery Mode, and Flash ECC (8 bits per 64-bit word, detecting 2-bit and correcting 1-bit errors, crashing via NMI on the assumption that a fault is an attack such as UV-C exposure)."
collection: "coldcard"
adapter: git
upstream_id: "docs/security-model.md"
upstream_type: git-file
revision: "43b2139227149c281141d08c612afd13c434d456"
sha: "03db46e73df133100a91c6b9f196a746b8211fdc"
canonical_url: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/security-model.md"
...
Learn more about the [Mk4's dual secure elements.](https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/secure-elements.md)
...
Upstream defect corrected: the body linked `mk4-secure-elements.md`, which does not exist at this revision. The unambiguous intended target is `docs/secure-elements.md` (the dual-secure-element document), so the destination now points there. The visible link text is untouched.
