---
title: "Coldcard Encrypted Backup File Format"
source: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/backup-files.md"
type: articles
ingested: 2026-08-10
tags: [coldcard, backup, 7z, aes-256, trng, bip-39-passphrase, seed-vault, recovery, entropy]
summary: "Describes the full-backup feature and its file format. A backup is a standard 7z archive with AES-256 encryption in CBC mode, where the 256-bit key is a SHA256 hash of a passphrase derived in a 7z-compatible way; the passphrase is 12 words chosen from the BIP-39 list by the device's TRNG, giving roughly 132 bits of entropy, so the argument for security rests on key length rather than on 7z's 16-byte salt, random 16-byte IV and few tens of thousands of key-stretching rounds. Because the container is a standard format, any AES256-SHA256-capable 7z tool can open it - the words joined by single spaces in lowercase are the passphrase - which the document presents as a deliberately verifiable design. Inside is one plain text file with everything needed to recover funds in another wallet; before version 4.0.0 it was always named ckcc-backup.txt, but the name is now random. Also covers BIP-39 passphrase handling (the default backs up the main wallet, with opt-in passphrase-wallet backup from 5.2.0), ephemeral seeds, limitations, internal filenames, and a fully worked example including the encrypted file, archive contents, the decryption procedure and the annotated contents of ckcc-backup.txt with its private-key, firmware-version and user-preference sections."
collection: "coldcard"
adapter: git
upstream_id: "docs/backup-files.md"
upstream_type: git-file
revision: "43b2139227149c281141d08c612afd13c434d456"
sha: "81b146c15dd9009a44a0546461918f14183a8664"
canonical_url: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/backup-files.md"
...
You can grab the [example file here](https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/backup.7z) and test it yourself, or use
