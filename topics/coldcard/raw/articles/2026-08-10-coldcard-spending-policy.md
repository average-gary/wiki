---
title: "Coldcard Spending Policy (autonomous signing with limits)"
source: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/spending-policy.md"
type: articles
ingested: 2026-08-10
tags: [coldcard, spending-policy, ccc, multisig, bypass-pin, velocity-limit, whitelist, web2fa, delta-mode, travel]
summary: "Describes the Spending Policy mode, which refuses to sign transactions exceeding a policy defined in advance and disables large parts of the device while active - suggested for travelling with a Coldcard. Distinguishes the single-signer version (the document's focus) from the multisig mode formerly called CCC, where the Coldcard acts as a co-signer that signs only when the policy is met while other signers decide independently; both can be active at once, and a transaction the multisig policy would sign is assumed acceptable for the main key too. Lists what a live policy costs: firmware updates blocked, no backups, Seed Vault and Secure Notes read-only or hidden, Settings inaccessible, and optionally BIP-39 passphrases blocked - hence the advice to fully configure the device first. Setup runs through Advanced/Tools > Spending Policy, defines the all-important Bypass PIN (the only way to disable the policy once enabled), then the policy itself: Max Magnitude in BTC per transaction, Velocity as a minimum time gap between signatures, a whitelist of up to 25 destination addresses (empty means any), and optional phone enrollment for Web2FA. Also covers the Word Check, Allow Notes and Related Keys switches, Last Violation, Remove Policy, Test Drive and ACTIVATE menu items, and a Tips and Tricks section spanning Money Manager Mode, passphrase considerations, Trick PIN interactions, locking out policy changes, Unlock Policy & Wipe, Delta Mode interaction, and what happens when the Bypass PIN is forgotten."
collection: "coldcard"
adapter: git
upstream_id: "docs/spending-policy.md"
upstream_type: git-file
revision: "43b2139227149c281141d08c612afd13c434d456"
sha: "693819834b5a7584607f64156ffc6f3501143f66"
canonical_url: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/spending-policy.md"
