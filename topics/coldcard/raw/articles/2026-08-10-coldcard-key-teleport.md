---
title: "Key Teleport: Encrypted Secret Transfer Between Coldcard Q Devices"
source: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/key-teleport.md"
type: articles
ingested: 2026-08-10
tags: [coldcard, key-teleport, ecdh, aes-256-ctr, bbqr, nfc, qr-codes, psbt, multisig, seed-vault]
summary: "Specifies Key Teleport, which moves small quantities of very secret data between two Coldcard Q systems such that nothing in the middle learns the secret. The receiver generates an EC keypair, stores it in settings and publishes the pubkey by QR or NFC, encrypted under a short 8-digit numeric code sent over a different channel; the sender does ECDH to derive a session key, picks an independent human-readable secret (the P key), AES-256-CTR encrypts the payload under the P key, then encrypts and MACs it under the session key. The receiver reconstructs the session key by ECDH, is prompted for the P key, and the decoded secret lands in the Seed Vault or secure notes as appropriate, after which the transfer keypair is destroyed. A multisig PSBT variant needs no receiver action: the sender uses the pubkey derived from the pre-shared XPUB already in the multisig wallet and the receiver drops straight into signing. Documents the leading data-type byte (s for words/raw master/xprv, x for full XPRV, n for notes export, v for seed vault export, p for a possibly-multisig PSBT, b for a complete system backup), the BBQr details (always used even for short payloads, uncompressed and Base32 because it is generated on the embedded device, with new type codes defined for this application), PSBT key selection, encryption details, the receiver password, the web component, UX details, and a security discussion addressing why such short passwords are acceptable and whether saving a QR image to the cloud is safe. Limits: about 4k of encoded data due to HTTP constraints, all transfers are data-typed and decode only on a Coldcard, and the Q model is required because QR codes are the final input path."
collection: "coldcard"
adapter: git
upstream_id: "docs/key-teleport.md"
upstream_type: git-file
revision: "43b2139227149c281141d08c612afd13c434d456"
sha: "9056e2dd84fe107b9771f9e80cb138a3fb36f0d1"
canonical_url: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/key-teleport.md"
