---
title: "How Coldcard's Dual Secure Elements Work Together"
source: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/secure-elements.md"
type: articles
ingested: 2026-08-10
tags: [coldcard, secure-element, atecc608, ds28c36b, key-hierarchy, pairing-secret, pcrop, aes-256-ctr, hmac-sha256, rate-limiting]
summary: "Deep technical account of the Mk4/Mk5/Q dual-secure-element scheme: SE1 (Microchip ATECC608) and SE2 (Maxim DS28C36B), both using secp256r1 and HMAC-SHA256 but deliberately sourced from different vendors so that a bug in one is unlikely to affect the other. Assumes an attacker who has opened the case, can probe the buses, de-solder parts and mount an active man-in-the-middle. Documents the 32-byte Pairing Secrets held in bootloader-only protected flash (guarded by the STM32 internal firewall plus PCROP), with a separate unique secret per element and all commands XOR'ed with HMAC-SHA256. The seed is AES-encrypted inside SE1 under a key split across the MCU and SE2, requiring an ECC signature, an ECDH exchange and the main PIN to reassemble: k = HMAC-SHA256(key=mcu hmac key, msg=SE2 easy key + SE2 hard key + current replaceable mcu key), used as AES-256-CTR with the MAC formed from 32 appended zeros in keyslot 10 and a nonce of the first 15 bytes of the MCU HMAC key plus a zero. Includes the full 15-row key table, the 13-PIN-attempts-restored-on-success rule, the 256 replaceable MCU keys (one consumed per wipe, the basis of Fast Wipe), the Trick PIN layout across SE2's even pages with secrets on odd pages, Spare Secrets (3x72 bytes) and the Long Secret (416 bytes), and the deliberate design choice that SE2 cannot validate a PIN at all - it has no rate limiting and a 6ms attempt rate, so all PIN rate-limiting responsibility rests on SE1. Ends with Fast Brick, implemented by rotating SE1's pairing secret."
collection: "coldcard"
adapter: git
upstream_id: "docs/secure-elements.md"
upstream_type: git-file
revision: "43b2139227149c281141d08c612afd13c434d456"
sha: "44def43ef7b7291e32ea709f45c1756ea8965404"
canonical_url: "https://github.com/coldcard/firmware/blob/43b2139227149c281141d08c612afd13c434d456/docs/secure-elements.md"
