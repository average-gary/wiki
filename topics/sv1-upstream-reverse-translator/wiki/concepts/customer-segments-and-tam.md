...
confidence: medium
sources:
  - raw/articles/2026-05-28-path5-customer-segments.md
  - raw/articles/2026-05-28-path5-luxor-hashrate-routing.md
  - raw/articles/2026-05-28-path5-demand-pool-and-easy-sv2.md
  - raw/articles/2026-05-28-path5-pool-software-landscape.md
...
tags: [tam, customer-fit, business-motivation, market-sizing, hashrate-broker, hashrate-rental]
summary: "Why would anyone deploy a reverse translator in 2026? Path 5's market analysis with verdict per hypothesis."
---
...
| 3 | Hashrate brokers fanning SV2-front to SV1 backends | **Supported (upgraded 2026-07-27)** — was "unclear, leaning aspirational" | A hashrate-rental proxy shipped with exactly this translation, in production: [[prior-art-blitzpool-rental-proxy]]. Luxor still shows zero public SV2 work. |
...
4. **Hashrate brokers (Luxor, NiceHash, OKMiner)** — *and, as of 2026-07-27, small independent rental operators*. Architectural fit confirmed by a shipped implementation (below); commercial pull at the large end still absent. Was ranked "2027-2028 customer if SV2 reaches escape velocity"; the small end arrived early.
5. **Solo miners / OCEAN customers / p2pool**. *Not* customers. They want to avoid centralized pools, not bridge to them.

## Segment #4 has a real instance (2026-07-27)

The hashrate-broker hypothesis was the weakest in the original read: architecturally obvious, commercially unevidenced. It now has an existence proof, though not from the direction expected.

`warioishere/blitzpool-rental-proxy` is a deployed hashrate-rental proxy whose whole purpose is routing a seller's miner to a buyer's chosen upstream — and it implements SV2-miner → SV1-pool translation to do it ([[prior-art-blitzpool-rental-proxy]]). The economics are the ones this segment predicted: the proxy measures share rate × difficulty at the wire, rented shares credit the buyer's upstream account, the seller earns a rental fee rather than the mining reward, the operator takes a margin, and billing is quantified in **TH·day**.

What updates and what does not:

- **The architecture-market fit is confirmed.** Somebody with a rental business needed exactly this translation and built it. That is the segment's thesis, realized.
- **The commercial scale is not.** This is a single-author project with 0 stars and 0 forks, sibling to a small pool (`blitzpool-server-rust`, ingested under [[../../bitcoin-mining-payout-schemas/_index|bitcoin-mining-payout-schemas]]). It is evidence that the *shape* is real, not that revenue is.
- **The realized customer is smaller than modeled.** The original read watched Luxor and NiceHash. The actual builder is an independent operator on a self-hosted pool stack. If that generalizes, segment #4's near-term surface is a long tail of small rental/self-hosting operators rather than a handful of large brokers — more customers, far less hashrate each, and a population that builds rather than buys.
- **It sharpens the "developer tool first" bottom line rather than overturning it.** Blitzpool did not adopt a reverse translator; it *wrote one*, out of tree, because none existed. That is still a tooling gap, just one now demonstrably worth an outsider's engineering budget.

The verdict on hypothesis 3 moves from "unclear, leaning aspirational" to **supported at small scale, unevidenced at large scale**. This article's `confidence` drops to medium as a result: the segment ranking rests on a market read that has now been partially falsified in its details once, and could be again.
...
- **More independent rental/self-hosting operators shipping their own SV2→SV1 translation** (as blitzpool did) → confirms the long-tail-of-builders read and makes an upstream `stratum_translation` contribution the highest-leverage move available, since each of them is currently paying the same cost separately.
...
- [[prior-art-blitzpool-rental-proxy]] — segment #4's first real instance
