...
## [2026-08-17] publish | "The vectors run in a browser with no toolchain — Pyodide + vendored secp256k1lab, and why that is reproducibility rather than independence" → ~/repos/sp-coinbase-vectors (working tree, uncommitted at time of writing; new `vendor/`, `test_wasm.mjs`, `.nojekyll`, `index.html` §8)

The review barrier is gone. Until this round, checking the coinbase silent-payments vectors meant cloning two repos and having Python 3; now <https://average-gary.github.io/sp-coinbase-vectors/> runs the **real** `sp_coinbase.py` and `run_tests.py` on CPython 3.14.2 compiled to WebAssembly, on a button press, with no toolchain at all. Pyodide `v314.0.4` is fetched from jsDelivr and SRI-pinned (`index.html:661-662`) and the curve maths comes from a new `vendor/` holding byte-identical copies of `secp256k1lab` (MIT), `bitcoin_utils.py` (BSD-2-Clause) and `ripemd160.py` (MIT), licences and attribution recorded in `vendor/README.md`. `sp_coinbase.py:57-70` now puts the bips clone *and* `vendor/` on `sys.path` in that order — nonexistent entries are silently skipped, so one ordering serves both worlds: locally the clone wins and the suite executes upstream's own module objects, in the browser the vendored copies do. Verified 2026-08-17: local full run is baseline + 11 cases + 3 new guards all `PASS` in 66 s with the bips clone byte-identical, and `test_wasm.mjs` gets 9 cases green under Pyodide with no clone in reach.

**The finding worth keeping is epistemic, and it cuts against the obvious reading of a green tick in a browser: running the same Python in WebAssembly buys reproducibility, not independence.** A second execution of one implementation reproduces its author's misreadings faithfully; nothing about a different interpreter changes that. What carries independence here is the **separate JavaScript reimplementation** of case 12 in `index.html` §7 — ~40 lines of `BigInt` secp256k1 plus `crypto.subtle` SHA-256, written from the displayed formulas — which landed in commit `a90ecc5` and was never logged. It earned the description: **it found a real bug.** `B_spend` was being decompressed by lifting its x-coordinate to the even-Y root and discarding the `02`/`03` parity prefix, so an odd-Y spend key yielded the wrong point. **Nine of the ten values still matched and only `P_0` diverged** — the exact signature of a serialization-convention error, and precisely the failure class that a re-run of the same code, in any interpreter, cannot surface. Fixed at `index.html:547` (`decompress()` now honours the prefix). So the WASM work is additive and the JS demo must not be "superseded" by it: `README.md:143-150` cites the JS rewrite as one of only three things breaking the vectors' self-consistency, and `run_tests.check_html_vector_in_sync()` pins its hardcoded copy of case 12 against the JSON so it cannot rot.

**What the page proves is bounded, and the boundary is stated on the page rather than buried.** Two of the eleven cases plus the 28-vector baseline execute **unmodified upstream `reference.py`**, which is deliberately *not* vendored — a local copy could not evidence agreement with the original, which is the only thing it exists to evidence, and `check_vendored_files_pristine()` asserts `vendor/reference.py` does not exist. So the browser establishes recomputation of every recorded number from the givens, that each miner's key spends its own output, and that the negative controls fail exactly as claimed; it does **not** establish agreement with BIP 352 itself. That asymmetry is encoded in the exit code (`run_tests.py:537`): checks that cannot run raise `Unavailable` and report `SKIP`, and a clone-less run at a *terminal* exits **1** even with everything green, because nothing in it was checked against upstream (`--allow-skips` to accept a partial run), while under `sys.platform == "emscripten"` it exits 0 since there the absence of a clone is structural rather than a mistake. Absence is always *detected*, never declared by a flag, so there is no flag to lie with.

**Vendored-vs-upstream drift is guarded four ways**, on the reasoning that a stale `vendor/` means the published page showing green ticks over code nobody reviewed — the same false-green failure the `index.html` sync check already existed to prevent. (1) `check_vendored_files_pristine()` (`run_tests.py:385`) sha256-compares raw bytes of every vendored file against the clone's **current HEAD**, not the commit pinned in `vendor/README.md`, so upstream moving fails loudly; it also sweeps for unlisted `.py` files, since a per-file loop alone passes vacuously on an emptied directory. (2) A subprocess import with `BIPS_REPO=/nonexistent` (`run_tests.py:425`), because byte-identical files can still be an *incomplete* set — a new import in `sp_coinbase.py` would break in the browser and nowhere else. (3) `check_page_pyodide_file_list()` (`run_tests.py:460`) asserts the page's `PY_FILES` entries are relative, git-tracked and cover the required set. (4) `test_wasm.mjs` mounts **the page's own** `PY_FILES` list rather than a second copy, and asserts `index.html` pins the same Pyodide version as the installed npm build — otherwise the test would verify a build no reader loads.

Four deployment facts learned the hard way, all of which fail only in a real browser. **GitHub Pages runs Jekyll, which excludes `_`-prefixed paths**, so without a committed `.nojekyll` the page 404s on `vendor/secp256k1lab/__init__.py` and nothing else — `run_tests.py:492` asserts the file is tracked. **It is a project page**, so every fetch must be relative; a leading `/` resolves outside `/sp-coinbase-vectors/` and 404s. **Pyodide raises `OSError` errno 138 ("emscripten does not support processes"), not `FileNotFoundError`**, so the git-availability probe has to catch `OSError`. And the **SRI hash covers the 19 KB loader only** — the ~6 MB of wasm and stdlib that loader then fetches itself carries no integrity check, which is stated in a comment rather than implied away. Status at time of writing: §7 is live at `a90ecc5` and §8 plus `vendor/` are verified locally but **not yet pushed** (confirmed 2026-08-17 by curl: the live page has no `PY_FILES` and `vendor/secp256k1lab/__init__.py` 404s), so the two-ways claim goes true on the parent commit. Inventory updated: `coinbase-native-stealth-payout` gained a § Runnable in a browser and a `next_action` that now names the Pages URL, records the 9-of-11 browser coverage, and explicitly warns the reader not to read the WASM run as independent verification — the spec diff remains the only outstanding item, unchanged in substance for the third round running. `inventory/_index.md` freshness dates were **not** touched this round and need a bump.

## [2026-08-24] ll | "Who controls the SV2 coinbase, and why address rotation doesn't hide amounts" → raw/notes/2026-08-24-ll-jd-coinbase-control-amount-linkability.md (7 lessons, 2 articles updated)

Seven lessons from reviewing four drafts of a blog post on the coinbase silent-payments variant and investigating `sv2-spec` PR #203. Two are corrections to claims this wiki and the published explainer had made, and **none of them was catchable by the test suite** — it stayed green throughout, because none are arithmetic errors. They are errors about *what a mechanism is for* and *what a leak actually leaks*, the class only argument surfaces.

**The nonce protects the receiver (lesson 1).** User challenge, from reading BIP 352's *"Using all inputs"*: maybe SPc doesn't need a nonce, since a public pool has no sender privacy to protect. The read of that section was right and its rationales genuinely are void for us — scanning cost and light clients don't apply to an input-less coinbase, and the CoinJoin case (*"If Alice uses a random input… this necessarily reveals to Bob which input Alice has control of"*) presupposes an anonymity set a known pool doesn't have. But the nonce is not from that section. It is from *"Preventing address reuse"* (`:88`), and its beneficiary is the **receiver**: the address at risk is the miner's. The `why_smallest_outpoint` footnote states the purpose flatly. So the explainer's framing — which never said whose privacy was at stake — invited precisely the wrong inference, and has been rewritten. Recorded alongside it: rotating `a_send` and mixing in the height are **overlapping** freshness sources, either alone sufficient, so the height *is* redundant for that job. It stays because it is free and unique **by consensus** rather than by the pool rotating correctly — stale key state after a restart, failover to a spare with an old batch, or an off-by-one batch index would otherwise reuse every payout address for every miner with no on-chain signal.

**Anti-grinding comes from committing to `A`, and three drafts in a row wired it to the height (lesson 2).** The `why_include_A` attack needs `input_hash′` known *before* choosing `a′ = input_hash·a / input_hash′`. A hash over the nonce alone lets the attacker pick any nonce, compute, and solve directly — the freely-chosen nonce is the attack's **lever, not its defense**. Committing to `A` makes the hash depend on the key being chosen, turning a solve into a fixed-point search. Sharpened for this setting: a pool picks a fresh `a_send` every block anyway, so it already holds the freedom the attack needs.

**"Who controls the SV2 coinbase" has no single answer (lesson 3), and I answered too bluntly twice before getting it right.** Three layers, each with a different answer: (a) base JDP negotiates only *where the pool gets paid* — §6.4.3 reserves the **first** output as the pool payout output, requires JDC to fund it, and constrains further pool-added outputs to **0 value**, so value flows to one output and distribution happens elsewhere; the user's objection that *something* must be negotiated was correct and the direction is simply inverted from what SPc needs. (b) **PR #203** (extension `0x0003`, open, supersedes closed #195/#202) reverses it — the pool publishes `SetPayoutDistribution` with per-miner outputs and relative weights, amounts computed once `T` is known. (c) The two JD modes differ on the scriptSig and **one cannot host a pool tag at all**: Full-Template (`DeclareMiningJob` §6.4.4) carries prefix+suffix so the pool sees and may reject it, while Coinbase-only (`SetCustomMiningJob` §5.3.18) carries only `coinbase_prefix` (≤8 B height), `nSequence`, outputs, locktime and merkle_path — no arbitrary scriptSig bytes, so the pool can neither specify nor validate a tag. Neither mode has a pool → JDC field for scriptSig content. Consequence: the `OP_RETURN` in `additional_outputs` is not a fallback but the only carrier working in both modes with no new fields, and it costs ~43 B / ~172 WU of real block space — so the "bytes are pre-paid, it costs nothing" argument holds for pool-controlled templates and **not** under JD.

**You cannot hide coinbase outputs from the miner who hashes them (lesson 4).** Proposed fix was to give the pool more authority over `coinbase_tx_prefix`. It cannot work, for a reason structural to mining rather than to the spec: the extranonce sits inside the scriptSig, so every byte after it is re-hashed per roll and must be held in the clear. SHA-256 being Merkle–Damgård, the **prefix** could collapse to a midstate; the **suffix** cannot, because it follows the varying bytes — and the outputs are in the suffix. The bytes one would most want to withhold are exactly those that must be handed over. The only real dial is payout-slot count, which #203 §9.2 already names.

**Walked back an overstated leak (lesson 5).** I had claimed a JDC retaining `SetPayoutDistribution` "holds the payout mapping" and kills forward secrecy — told to the user as grounds for reframing the post, then corrected the next turn. It carries `(script, weight)` pairs and **no identities**; the script→miner mapping is in the pool's DB, so erasure still does its job. Worse for the claim, weights are near-redundant with public data since `amount[i] = floor(weight[i]·T/W)`. The genuine residue is narrower: distributions for blocks **nobody won**, and **sub-dust miners** pruned from the coinbase and so invisible on-chain.

**The ceiling was already in this wiki (lesson 6).** Asked for "huge gotchas" under the premise that interactivity lets engineering fix anything. Four of five frictions are indeed engineering-solvable (grace-window height mismatch → scan a small candidate window; per-height distribution refresh; the JD carrier; pre-block verification over the existing encrypted session). The fifth is not, and is not addressed by SPc at all: **amounts stay cleartext**, so `amount_i / T` fingerprints a stable hashrate share across blocks with no address ever repeating, and heavy-tailed pool hashrate makes the largest providers easiest to track. `wiki/concepts/coinbase-amount-linkability.md` § *The ceiling on what amount-hiding can achieve* had this documented with the WabiSabi bound — reached for only after being asked, after a week spent on the address channel.

**Pitch a protocol change so it stands without your proposal (lesson 7).** User's decomposition — a small separate extension for the pool tag, distinct from #203's payout distribution — is right on separation-of-concerns, but the stronger point is framing: a pool-required coinbase tag justifies itself with no reference to silent payments, because a JD-declared block today carries whatever tag the JDC chose, so a pool running JD **loses its coinbase attribution entirely**, and tags are how pool hashrate share is publicly measured. Pitched that way it needs no privacy argument bought first and SPc becomes a consumer rather than the justification.

Articles updated, append-only. `wiki/concepts/coinbase-amount-linkability.md` gained "### Correction to the section above — the impossibility argument does not hold": its § *"BIP 352 silent payments cannot rescue this — a hard structural no"* reasons from "no input private key ⇒ no `a` ⇒ no shared secret," which is sound about BIP 352 as written and unsound as an impossibility result, since it assumes `a` must come from the inputs. The "zero occurrences of coinbase in BIP 352" finding stands and is the actual point — unspecified, not impossible. The article's real thesis survives intact and is now stated: silent payments rescue *address* linkability, never *amount* linkability. `wiki/concepts/pplns-jd.md` gained "## Non-custodial payouts under JD: who actually controls the coinbase" recording the ordering constraint, #203's weight-based resolution, the two-mode scriptSig asymmetry, §9.2's acknowledged leak, and the midstate impossibility — filed there because #203 explicitly targets non-debt accounting (PPLNS, SLICE, TIDES) and says PPS/FPPS SHOULD NOT use it.

No new inventory record: the follow-ups belong to the existing `coinbase-native-stealth-payout` candidate, and the one durable correction (the superseded impossibility section) is now recorded in the article itself rather than as a tracked task.
